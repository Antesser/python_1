...
def get_last_logfile_inf(log_dir: str | Path) -> Optional[LogfileInf]:
...
def select_last_logfile(files: List[str]) -> Optional[LogfileInf]:
    filename_pattern = (
        r"^nginx-access-ui.log-(19\d\d|20\d\d)([01]\d)([0-3]\d)(\.gz|)$"
    )
    pattern_obj = re.compile(pattern=filename_pattern)

    result = LogfileInf(path=None, date=datetime.date.min, type=None)
...
def parse_logfile(
    ...
    with open(
        logfile_inf.path, "rb"
    ) if logfile_inf.type is LogfileType.PLAIN else gzip.open(
        logfile_inf.path, "r"
    ) as fd:
...
def get_logfile_stats(
    ...
    for inf in logfile_parser(logfile_inf):
        ...
        if inf is None:
            err_lines += 1
            continue
        if stats.get(inf.URL) is None:
            stats[inf.URL] = URLStats(
                count=1,
                time_avg=0.0,
                time_max=inf.request_time,
                time_sum=inf.request_time,
                url=inf.URL,
                time_med=0.0,
                time_perc=0.0,
                count_perc=0.0,
            ...
            stats[inf.URL] = stats[inf.URL]._replace(
                count=stats[inf.URL].count + 1,
                time_sum=float(
                    format(stats[inf.URL].time_sum + inf.request_time, ".3f")
                ),
                time_max=max(stats[inf.URL].time_max, inf.request_time),
            )
        summary_time += inf.request_time
        req_times[inf.URL].append(inf.request_time)
...
def main(config: Dict[str, Any]) -> None:
    ...
    try:
        last_logfile_inf = get_last_logfile_inf(log_dir=config.get("LOG_DIR"))

        if last_logfile_inf is None:
            logging.info(
                "logfiles were not found in '%s'", config.get("LOG_DIR")
            )
            return

        logging.info("last logfile found: %s", last_logfile_inf.path)

        if report_already_exists(
            report_path=report_path, date=last_logfile_inf.date
        ):
            logging.info(
                "Report for '%s' has already been created in '%s'",
                last_logfile_inf.path,
                report_path,
            )
            return
        ...
        new_report_path = report_path.joinpath(
            report_date(date=last_logfile_inf.date)
        )
